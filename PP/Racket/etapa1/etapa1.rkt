...
(define (cauta st pattern copy-pattern) ;;functie care cauta patternul in eticheta, adica verifica daca patternul este prefixul comun
  (if (null? st)
    copy-pattern
    (if (null? pattern)
      copy-pattern
...
(define (sufix st pattern suf) ;;functie care returneaza sufixul
  (if (null? st)
    suf
    (if (null? pattern)
      suf
