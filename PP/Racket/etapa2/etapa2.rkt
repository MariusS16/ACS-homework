...
(define (get-ch-words words ch)
  (filter (lambda (word) (and (not (null? word)) (equal? (car word) ch))) words))
...
(define (ast-func suffixes)
  (reverse 
    (foldl (lambda (suffix y) (cons (cdr suffix) y)) (list (list (car (car suffixes)))) suffixes)))
; (define (ast-func suffixes)
;   (cons (list (car (car suffixes))) (map (lambda (suffix) (cdr suffix)) suffixes)))
...
(define (cst-func suffixes)
  (reverse 
    (foldl ;foldl (lambda prefix listaSUF)
      (lambda (suffix acc) 
        (cons (drop suffix (length (longest-common-prefix-of-list suffixes))) acc))
      (list (longest-common-prefix-of-list suffixes))
      suffixes)))
; (define (cst-func suffixes)
;   (cons (longest-common-prefix-of-list suffixes) 
;     (map (lambda (suffix) (drop suffix (length (longest-common-prefix-of-list suffixes)))) suffixes))) ;pentru fiecare sufix, eliminam prefixul comun
...
(define (suffixes->st labeling-func suffixes alphabet)
  (apply append 
    (map (lambda (ch) ;pt fiecare caracter din alfabet
        (cond
          ((not (null? (get-ch-words suffixes ch)))
            (let ((new-suf (labeling-func (get-ch-words suffixes ch)))) 
              (list (cons (car new-suf) (suffixes->st labeling-func (cdr new-suf) alphabet))))) ;daca exista sufixe cu caracterul ch, le procesam
          (else '()))) 
      alphabet)))
...
(define (text->st labeling-func)
  (lambda (text)
    (let ((n-s (get-suffixes (append text (list #\$))))) ;prima - obtinem suf si adaugam $ la final
      (suffixes->st labeling-func n-s (remove-duplicates (sort (append text (list #\$)) char<?))))));2-3 - obtinem alfabetul sortat
