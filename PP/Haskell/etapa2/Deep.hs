...
toTransformation transformation = 
    case transformation of
        ...
        Combine tf -> S.combineTransformations $ foldr (\x acc -> (toTransformation x):acc) [] tf
...
toRegion region = 
    case region of
        ...
        Complement r1 -> S.complement (toRegion r1)
        Transform tr r -> S.applyTransformation (toTransformation tr) (toRegion r)
...
decomposeTransformation transformation = 
    case transformation of
        ...
        Combine tf -> foldr (\x acc -> (decomposeTransformation x) ++ acc) [] tf
{-
    *** TODO ***
...
fuseTransformations = foldr (\x acc ->
    case (x, acc) of
        (Translation x1 y1, Translation x2 y2 : ts) -> Translation (x1 + x2) (y1 + y2) : ts
        (Scaling s1, Scaling s2 : ts) -> Scaling (s1 * s2) : ts
        _ -> x : acc) []
...
optimizeTransformations region = 
    case region of
        ...
        Circle r -> Circle r
        Union r1 r2 -> Union (optimizeTransformations r1) (optimizeTransformations r2)
        Intersection r1 r2 -> Intersection (optimizeTransformations r1) (optimizeTransformations r2)
        Complement r1 -> Complement (optimizeTransformations r1)
        Transform tr r -> 
            case tr of
                Combine tf ->  -- Dacă transformarea este o combinație de transformări
                    case r of
                        Transform tr1 r1 ->  -- Dacă regiunea este o transformare
                            case tr1 of
                                _ -> Transform (combineTransformations (fuseTransformations (decomposeTransformation (Combine (reverse (tr1 : (reverse tf))))))) (optimizeTransformations r1) 
                        _ -> Transform (combineTransformations (fuseTransformations (decomposeTransformation (Combine tf)))) r 
                _ -> Transform tr (optimizeTransformations r) -- Dacă transformarea nu este o combinație de transformări