...
rectangle width height (x, y) 
    | x <= halfwidth && -halfheight <= y && y <= halfheight && -halfwidth <= x = True
    | otherwise = False
    where
        halfwidth = width / 2
        halfheight = height / 2
...
plot width height region = intercalate "\n" [concat [if inside (x, y) region then "*" else "." | x <- x_width] | y <- y_height]
    where
        f_width = fromIntegral width
        f_height = fromIntegral height
        x_width = [-f_width .. f_width]
        y_height = [f_height, f_height - 1 .. -f_height]
...
promoteBinary f pointed1 pointed2 point = result1 `f` result2
    where
        result1 = pointed1 point
        result2 = pointed2 point
