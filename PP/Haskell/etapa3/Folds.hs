...
union r1 r2 = R $ Union r1 r2

intersection :: RegionAST -> RegionAST -> RegionAST
intersection r1 r2 = R $ Intersection r1 r2
...
{-
    ...
    * un nod copil este indat cu două spații față de părintele său
    ...
    * Definiți o funcție ajutătoare care ia ca parametru nivelul curent
      din arbore, pentru a ști câtă indare să aplicați.
...
instance Show RegionAST where
    show (R region) = func 0 region

func :: Int -> RegionShape RegionAST -> String
func lvl region = 
    case region of
        Circle r -> ind ++ "Circle " ++ show r
        FromPoints pt -> ind ++ "FromPoints " ++ show pt
        Rectangle x y -> ind ++ "Rectangle " ++ show x ++ " " ++ show y
        Union (R r1) (R r2) -> ind ++ "+" ++ "\n" ++ func add r1 ++ "\n" ++ func add r2
        Intersection (R r1) (R r2) -> ind ++ "*" ++ "\n" ++ func add r1 ++ "\n" ++ func add r2
        Complement (R r) -> ind ++ "~" ++ "\n" ++ func add r
        Transform transformation (R r) -> ind ++ show transformation ++ "\n" ++ func add r
    where
        ind = help_ind lvl
        add = lvl + 1

help_ind :: Int -> String
help_ind lvl = replicate (lvl + lvl) ' '
...
instance Functor TransformationShape where
    ...
    fmap f transformation =
        case transformation of
            ...
            Combine tfs -> Combine $ fmap f tfs
...
instance Functor RegionShape where
    ...
    fmap f region = 
        case region of
            ...
            Transform transformation r -> Transform transformation $ f r
...
foldTransformationAST f (T transformation) = 
    case transformation of
        Scaling x -> f (Scaling x)
        Translation x y -> f (Translation x y)
        Combine tfs -> f $ Combine (fmap (foldTransformationAST f) tfs)
  ...
  where
    ...
    combiner transformation = 
        case transformation of
            ...
            Combine tfs -> S.combineTransformations tfs
...
{-
    *** TODO ***
    ...
    helper lvl region = ... helper (lvl + 1) subRegion ...,
    ...
    funcția (helper lvl) aplicată pe o regiune depinde nu de rezultatul
    aplicării aceleiași funcții (helper lvl) pe subregiuni, ci al altei
    funcții, (helper (lvl + 1)). Prin urmare, definiția de mai sus nu poate fi
    ...
    helper' region lvl = ... helper' subRegion (lvl + 1) ...
    ...
    regiune nu mai întoarce direct un șir, ci o funcție (\lvl -> string), care
